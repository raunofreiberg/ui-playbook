...
<div className="center">
  <TextField placeholder="Placeholder" style={{ width: 270, margin: '32px 0' }} />
</div>
...
This component should use the HTML [`<input>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input) element under the hood.
...
The input should visually change color, and be associated with an error label to announce the message to screen readers.

### Icon support

Icons can be useful to either describe the intent of the input, or provide additional functionality.

For example, a search input could come with a magnifying glass icon to indicate searchability.
...
For example, prefer "rauno@freiberg.com" instead of "Your e-mail address" for an email input.

### Associate a label with the input

While placeholders provide valuable guidance for users, they are not a replacement for labels.

Placeholders aren't visible to the user once the field has been filled in, nor are they announced by screen readers.

### Indicate required fields

A common pattern to indicate required fields would be to use the HTML [`required`](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/required) attribute
...
When validating input, be mindful of when the user is finished interacting with the field.

For complex input such as passwords, instant inline validation will prevent users from guesswork and reduce the amount of errors.

Whereas for simpler input where the expected data is more obvious (such as email), avoid showing the error until the user has finished typing.
...
Instead, consider using a text input with the [`pattern`](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/pattern) attribute like so:
...
In which case, [the label can be hidden visually](https://www.w3.org/WAI/tutorials/forms/labels/#hiding-label-text), but needs to remain accessible for users using assistive technologies.
