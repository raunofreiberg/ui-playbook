...
Indicates that the checkbox is partially selected and controls a set of sub-checkboxes.
...
A standalone checkbox without a label does not describe its function. Make sure to associate the checkbox with a visible and [accessible label](#provide-a-label-element).

### Do not trigger actions

Checkboxes should not be used to trigger immediate actions like enabling or disabling a feature.
Instead, request confirmation from the user before applying the changes.
...
HTML checkboxes used to be notoriously tricky to style. Not to mention the fact that each browser has its own appearance for the element, making it look and feel different across platforms.
...
In most cases, you will probably use the second option like so:

<label class="checkboxExample">
...
- Screen readers will read out the label when the checkbox is focused, making it easier for an assistive technology user to understand the function of the checkbox.

### Mixed checkbox

If a mixed checkbox is indeterminate and it's descendants are not checked, convey the state by placing `aria-checked="mixed"` on it.
