...
- Appear as a result of user input or general occurrences in the application.
...
For actionable notifications, the duration can be extended to <strong>10 seconds</strong> to give the user enough time to interact with it.
...
If a screen reader is currently reading something, using `alert` will wait for it to finish, whereas `status` would interrupt the announcement and read immediately. In most cases, you would use `status`.
